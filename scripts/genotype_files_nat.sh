...
CHR="$1"
STATE="$2"
WD="/mnt/rosalind_1/liri/puzzle_sdumont"
...
HAPS_DIR="$WD/$STATE/nat/chr_info_unfilt/haps_nat_teste" #haps separated by chr from anc
#HAPS_SIMPLIFIED_DIR=$HAPS_DIR/haps_simplified
VCF_DIR="$WD/$STATE/nat/chr_info_unfilt/vcf_nat_teste" #vcf all samples all chr from anc
    ...
    awk '{
        ...
        for (i = 6; i <= NF; i++) {
            if ($i == fourth_col) {
                ...
                # If the value does not match fourth_col or fifth_col, keep it unchanged 
...
for ID in $INDIVIDUALS; do
   ...
   for HAP in A B; do
    ...
    # Extract coordinates
   awk '{print $2, $3, $4}' "$WD/${ID}_${HAP}_${CHR}_${STATE}_fragments.txt" > "$WD/${ID}_${HAP}_${CHR}_${STATE}_coords.txt"

   rm "$WD/${ID}_${HAP}_${CHR}_${STATE}_fragments.txt"
    # Subset haps file for one hap of all NAT individuals
     while read -r chrom initial_pos final_pos; do
                        awk -v ip="$initial_pos" -v fp="$final_pos" -v hap="$HAP" '$3 >= ip && $3 <= fp {print $0}' "$HAPS_FILE" >> "$WD/${ID}_${HAP}_${CHR}_${STATE}_subset.haps"
     done < "$WD/${ID}_${HAP}_${CHR}_${STATE}_coords.txt"
   ...
   #Alleles in 0/1
   haps_alleles_to_0_1 "$WD/${ID}_${HAP}_${CHR}_${STATE}_subset_final.txt" "$HAPS_DIR/${ID}_${HAP}_${CHR}_${STATE}_haps_nat.haps"
