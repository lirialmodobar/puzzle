...
new title: occur_all_ancs_state.Rmd
...
rs_count <- rs_count[order(rs_count[,3]),]
...
rs_count <- rs_count[,c(1,2,4,3)]
colnames(rs_count) <- c("chrom", "start", "end", "score")
rs_count <- rs_count[order(rs_count$start),]
...
## EUR
read.table("/home/yuri/liri/puzzle/109_preliminar_oct23/eur/chr_info_unfilt/count_info/sort_pos_count_chr_1_eur.txt", h=F) -> count_eur_rs
count_eur_rs <- count_eur_rs[order(count_eur_rs[,3]),]
...
count_eur_rs <- count_eur_rs[,c(1,2,4,3)]
colnames(count_eur_rs) <- c("chrom", "start", "end", "score")

## AFR
read.table("/home/yuri/liri/puzzle/109_preliminar_oct23/afr/chr_info_unfilt/count_info/sort_pos_count_chr_1_afr.txt", h=F) -> count_afr_rs
count_afr_rs <- count_afr_rs[order(count_afr_rs[,3]),]
...
count_afr_rs <- count_afr_rs[,c(1,2,4,3)]
colnames(count_afr_rs) <- c("chrom", "start", "end", "score")

## NAT
read.table("/home/yuri/liri/puzzle/109_preliminar_oct23/nat/chr_info_unfilt/count_info/sort_pos_count_chr_1_nat.txt", h=F) -> count_nat_rs
...
## UNK
read.table("/home/yuri/liri/puzzle/109_preliminar_oct23/unk/chr_info_unfilt/count_info/sort_pos_count_chr_1_unk.txt", h=F) -> count_unk_rs
count_unk_rs <- count_unk_rs[order(count_unk_rs[,3]),]
...
count_unk_rs <- count_unk_rs[,c(1,2,4,3)]
...
annoYaxis(
    ...
    axisLine = FALSE, fontsize = 9 #n lembro se mudava fonte, mas acho que sim
)

## Plot y-axis label
plotText(
    label = "variants occurrences", x = 0.005, y = 2.5, rot = 90, #adjust x, y and fontsize
    fontsize = 9, fontface = "plain", just = "center",
    default.units = "inches"
)
...
## Annotate y-axis
annoYaxis(
    plot = signalplotEUR, at = as.character(seq(0, max_score+50, by = 50)),label=as.character(seq(0, max_score+50, by = 50)),
    axisLine = FALSE, fontsize = 9
)
...
## Annotate y-axis
annoYaxis(
    plot = signalplotAFR, at = as.character(seq(0, max_score+50, by = 50)),label=as.character(seq(0, max_score+50, by = 50)),
    axisLine = FALSE, fontsize = 9
)
...
## Annotate y-axis
annoYaxis(
    plot = signalplotNAT, at = as.character(seq(0, max_score+50, by = 50)),label=as.character(seq(0, max_score+50, by = 50)),
    axisLine = FALSE, fontsize = 9
)
...
## Annotate y-axis
annoYaxis(
    plot = signalplotUNK, at = as.character(seq(0, max_score+50, by = 50)),label=as.character(seq(0, max_score+50, by = 50)),
    axisLine = FALSE, fontsize = 9
)
...
legendPlot <- plotLegend(
    ...
    fill = c("black","darkblue","red","darkgreen", "darkgrey"),
    border = FALSE,
