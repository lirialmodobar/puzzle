...
files <- list.files(unfilt_frags_dir)
files <- files[-23:-27] #removing dirs and files that are not unfilt frags
...
cutoff_length <- round(cutoff_length, 0)
write.table(cutoff_length, paste0(unfilt_frags_dir, "cutoff_length.txt"), row.names = FALSE, quote = FALSE)
