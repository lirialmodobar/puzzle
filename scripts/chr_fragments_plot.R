...
states <- c("rs_sp", "rs", "sp")
labels <- c("nat", "eur", "afr", "unk")
colors <- c("#6caf5e", "#7297b4", "#c05040", "#919191")

#Create plot
create_fragments_plot <- function(label, label_dir, state, color, chr, n_rows) {
	...
	positions_file_path <- file.path(label_dir, paste0(positions_file_prefix, ".txt")
	# Read the data file
	dados <- read.table(positions_file_path, header = FALSE, sep = "\t")
	positions <- dados[2:4]
	...
	# Start PDF device
	pdf_file <- file.path(label_dir, graph_dir, paste0(n_rows,positions_file_prefix,"_frags_plot.pdf"))
	pdf(pdf_file)
	...
	kpAddLabels(kp, labels = paste("frag", label, state))
...
for (i in 1:length(states)) {
	state <- states[i]
	for (j in 1:lenght(labels)) {
		label <- labels[j]
		color <- colors[j]
		label_dir <- file.path(wd, state, label, "chr_info_filt")
		if (opt$run_as_test) {
		...
		if (is.null(opt$chromosome)) {
			chromosomes <- 1:22  # Use all chromosomes
		} else {
		...
		for (chr in chromosomes) {
			if (opt$run_as_test) {
				for (n_rows in rows) {
					create_fragments_plot(label, label_dir, state,  color, chr, n_rows)
				}
			} else {
