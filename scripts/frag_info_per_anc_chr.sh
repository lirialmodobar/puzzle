...
WD=/mnt/genetica_1/liriel/dados_sp_script_antigo
	...
	# Process each chromosome (1 to 22)
        for chr in {1..2}; do
                ...
                if [ -f $WD/"$chr"_"$label_lower"_temp.txt ]; then
                        awk '!seen[$3]++' $WD/"$chr"_"$label_lower"_temp.txt | awk 'NR==1 {a=$4; printf "%s\t%d\t%d\t%d\t%d\tNA\t%s\n", $1, $2, $3, $4, ($4-$3), $5; next} {printf "%s\t%d\t%d\t%d\t%d\t%d\t%s\n", $1, $2, $3, $4, ($4-$3), ($3 <= a) ? 0 : 1, $5; a=$4}' > "$ANC_DIR/chr_info_filt/chr_${chr}_${label_lower}_sort_filt_size_gap.txt"
                        awk 'NR==1 {a=$4; printf "%s\t%d\t%d\t%d\t%d\tNA\t%s\n", $1, $2, $3, $4, ($4-$3), $5; next} {printf "%s\t%d\t%d\t%d\t%d\t%d\t%s\n", $1, $2, $3, $4, ($4-$3), ($3 <= a) ? 0 : 1, $5; a=$4}' $WD/"$chr"_"$label_lower"_temp.txt > "$ANC_DIR/chr_info_unfilt/chr_${chr}_${label_lower}_sort_unfilt_size_gap.txt"
                fi
