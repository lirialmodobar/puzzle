...
tail +3 $WD/bhrc_haps_sample/127_BHRC_altura_chr21.sample | awk '{ print $2}' | sed "p" | tr '\n' '\t' > $WD/geno_columns.txt
...
tail -n +2 "$WD/geno_header_127_BHRC_altura_chr21.haps" | while IFS= read -r line; do
    # Split the line
    ...
    for ((i = 5; i < ${#fields[@]}; i++)); do
        id="${ids[$i]}"
        print_allele "$id" "$snp_id" "$i" "$line" >> $WD/teste
    done
done
