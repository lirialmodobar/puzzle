...
#awk '{
  ...
  #  }
 #   print modified_line
#}' $WD/bhrc_haps_sample/127_BHRC_altura_chr1.haps > $WD/output_file.txt

#cat $WD/header.txt $WD/output_file.txt | head -n 10  > $WD/geno_header_127_BHRC_altura_chr1.haps
...
#header=$(head -n 1 $WD/geno_header_127_BHRC_altura_chr1.haps)

#tail +2 $WD/geno_header_127_BHRC_altura_chr1.haps > $WD/temp.txt
...
#while read -r line; do
   ...
   #     id="${ids[$i]}"
  #      print_allele "$id" "$snp_id" "$i" "$line" >> $WD/geno_info_BHRC_altura_chr1.txt
 #   done
#done < $WD/temp.txt

#rm $WD/temp.txt

# First step: find haps data in collapse intervals
...
find_vars_within_pos_range $WD/chr_1_eur_sp_unfilt.txt $WD/geno_info_BHRC_altura_chr1.txt $WD/vamos_ver.txt
